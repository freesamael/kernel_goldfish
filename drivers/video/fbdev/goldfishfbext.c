...
static irqreturn_t goldfish_fbext_interrupt(int irq, void *dev_id)
{
	unsigned long irq_flags;
	struct goldfish_fbext *fb = dev_id;
	u32 status;
...
static int
goldfish_fbext_setcolreg(unsigned int regno, unsigned int red, unsigned int green,
		 unsigned int blue, unsigned int transp, struct fb_info *info)
{
	struct goldfish_fbext *fb = container_of(info, struct goldfish_fbext, fb);
...
static int goldfish_fbext_check_var(struct fb_var_screeninfo *var,
							struct fb_info *info)
{
...
static int goldfish_fbext_set_par(struct fb_info *info)
{
	struct goldfish_fbext *fb = container_of(info, struct goldfish_fbext, fb);
...
static int goldfish_fbext_pan_display(struct fb_var_screeninfo *var,
							struct fb_info *info)
{
	unsigned long irq_flags;
	int base_update_count;
	...
	if (fb->base_update_count == base_update_count)
		pr_err("goldfish_fbext_pan_display: timeout waiting for base update\n");
	return 0;
}

static int goldfish_fbext_blank(int blank, struct fb_info *info)
{
	struct goldfish_fbext *fb = container_of(info, struct goldfish_fbext, fb);
	switch (blank) {
...
static struct fb_ops goldfish_fbext_ops = {
	.owner          = THIS_MODULE,
	.fb_check_var   = goldfish_fbext_check_var,
	.fb_set_par     = goldfish_fbext_set_par,
	.fb_setcolreg   = goldfish_fbext_setcolreg,
	.fb_pan_display = goldfish_fbext_pan_display,
	.fb_blank	= goldfish_fbext_blank,
...
static int goldfish_fbext_probe(struct platform_device *pdev)
{
	int ret;
	struct resource *r;
	struct goldfish_fbext *fb;
	size_t framesize;
	...
	dev_info(&pdev->dev, "initializing goldfish_fbext\n");
	...
	dev_info(&pdev->dev, "platform_get_irq\n");
	...
	dev_info(&pdev->dev, "get_fb_config_from_format\n");
	...
	fb->fb.fbops		= &goldfish_fbext_ops;
	...
	fb->fb.screen_base = (char __force __iomem *)dma_alloc_coherent(
						&pdev->dev, framesize,
						&fbpaddr, GFP_KERNEL);
	dev_info(&pdev->dev, "allocating frame buffer %d * %d, got %p\n",
	...
	dev_info(&pdev->dev, "fb_set_var\n");
	ret = fb_set_var(&fb->fb, &fb->fb.var);
	if (ret)
		goto err_fb_set_var_failed;

	dev_info(&pdev->dev, "request_irq\n");
	ret = request_irq(fb->irq, goldfish_fbext_interrupt, IRQF_SHARED,
							pdev->name, fb);
	if (ret)
		goto err_request_irq_failed;

	dev_info(&pdev->dev, "goldfish_fbext_pan_display\n");
	writel(FB_INT_BASE_UPDATE_DONE, fb->reg_base + FB_INT_ENABLE);
	goldfish_fbext_pan_display(&fb->fb.var, &fb->fb); /* updates base */

	dev_info(&pdev->dev, "register_framebuffer\n");
...
static int goldfish_fbext_remove(struct platform_device *pdev)
{
	size_t framesize;
	struct goldfish_fbext *fb = platform_get_drvdata(pdev);
...
static const struct of_device_id goldfish_fbext_of_match[] = {
	{ .compatible = "generic,goldfish-fbext", },
	{},
};
MODULE_DEVICE_TABLE(of, goldfish_fbext_of_match);

static const struct acpi_device_id goldfish_fbext_acpi_match[] = {
	{ "GFSH0009", 0 },
	{ },
};
MODULE_DEVICE_TABLE(acpi, goldfish_fbext_acpi_match);

static struct platform_driver goldfish_fbext_driver = {
	.probe		= goldfish_fbext_probe,
	.remove		= goldfish_fbext_remove,
	.driver = {
		.name = "goldfish_fbext",
		.owner = THIS_MODULE,
		.of_match_table = goldfish_fbext_of_match,
		.acpi_match_table = ACPI_PTR(goldfish_fbext_acpi_match),
	}
};

module_platform_driver(goldfish_fbext_driver);
